...
@export_file("*.tres") var box_panel: String = this_folder.path_join("default_narrator_style.tres")

@export_subgroup("Color")
@export var box_color_use_global: bool = false
@export var box_color_custom: Color = Color.DARK_GRAY

@export_subgroup("Size")
@export var box_size: Vector2 = Vector2(550, 110)

@export_subgroup("Animation")
@export var box_animation_in: AnimationsIn = AnimationsIn.FADE_IN
@export var box_animation_out: AnimationsOut = AnimationsOut.FADE_OUT
...
func _apply_box_settings() -> void:
	var dialog_text_panel: PanelContainer = %TextPanel
	...
	sizer.size = box_size
	sizer.position = box_size * Vector2(-0.5, -0.5) - Vector2(0, get_viewport().size.y / 2)
	print(dialog_text_panel.has_theme_stylebox_override('panel'), " ", sizer.size)
